...
class LessonHandler
{
    ...
    public function handle(int $chatId, int $userId, string $text): void
    {
        ...
        $pattern = '/^(DA|DB|[ABC])\s+(\d+(?:\.\d+)?)(?:\s+(\d{4}-\d{2}-\d{2}))?$/i';

        if (!preg_match($pattern, $text, $matches)) {
            $this->telegram->sendMessage(
                $chatId,
                "❓ Не понимаю формат.\n\nПримеры:\n<b>A 1.5</b>\n<b>B 1</b>\n<b>C 0.5 2026-03-15</b>\n<b>DA 1</b> — A для детей\n<b>DB 1.5</b> — B для детей\n\nУровни: A, B, C, DA, DB"
            );
            return;
        }
        ...
        if ($hours <= 0 || $hours > 12) {
            $this->telegram->sendMessage($chatId, '❌ Количество часов должно быть от 0.1 до 12.');
            return;
        }
    ...
    {
        return (bool) preg_match('/^(DA|DB|[ABC])\s+\d+(?:\.\d+)?/i', trim($text));
    }
